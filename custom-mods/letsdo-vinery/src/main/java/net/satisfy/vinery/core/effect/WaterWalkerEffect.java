...
public class WaterWalkerEffect extends MobEffect
{
    ...
    @Override
    public boolean applyEffectTick(net.minecraft.server.level.ServerLevel level, LivingEntity pLivingEntity, int pAmplifier) {
        if (!(pLivingEntity instanceof Player player && player.isSpectator())) {
            ...
            if (pLivingEntity.isInWater()) {
                ...
                if (pLivingEntity.level() instanceof ServerLevel serverLevel) {
                    serverLevel.sendParticles(ParticleTypes.FALLING_WATER, pos.x(), pos.y() + 0.1D, pos.z(), 10, 0.2, 0.1, 0.2, 1.5);
                }
