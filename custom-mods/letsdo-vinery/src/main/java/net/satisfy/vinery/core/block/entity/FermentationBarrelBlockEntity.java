...
public class FermentationBarrelBlockEntity extends BlockEntity implements ImplementedInventory, MenuProvider {
    ...
    public void loadAdditional(net.minecraft.world.level.storage.ValueInput input) {
        ...
        this.fluidLevel = input.getIntOr("FluidLevel", 0);
        this.juiceType = input.getStringOr("JuiceType", "");
    }
    ...
    public void saveAdditional(net.minecraft.world.level.storage.ValueOutput output) {
        ...
        output.putInt("FluidLevel", this.fluidLevel);
        output.putString("JuiceType", this.juiceType);
    }
    ...
    public static void tick(Level world, BlockPos pos, FermentationBarrelBlockEntity blockEntity) {
        ...
        var recipeHolder = ((net.minecraft.server.level.ServerLevel) world).recipeAccess()
    ...
    private boolean isIngredient(ItemStack stack) {
        ...
        return ((net.minecraft.server.level.ServerLevel) level).recipeAccess().getRecipes().stream()
                .filter(h -> h.value() instanceof FermentationBarrelRecipe)
                .anyMatch(recipe -> ((FermentationBarrelRecipe) recipe.value()).getIngredients().stream().anyMatch(ingredient -> ingredient.test(stack)));
    }
