...
public class FermentationBarrelGuiHandler extends AbstractContainerMenu {
    ...
    private boolean isIngredient(ItemStack stack) {
        if (!(this.level instanceof net.minecraft.server.level.ServerLevel serverLevel)) return false;
        return serverLevel.recipeAccess().getRecipes().stream()
                .filter(h -> h.value() instanceof net.satisfy.vinery.core.recipe.FermentationBarrelRecipe)
                .anyMatch(recipe -> ((net.satisfy.vinery.core.recipe.FermentationBarrelRecipe) recipe.value()).getIngredients().stream().anyMatch(ingredient -> ingredient.test(stack)));
    }
