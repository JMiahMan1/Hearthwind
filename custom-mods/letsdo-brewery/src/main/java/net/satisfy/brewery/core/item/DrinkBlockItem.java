...
public class DrinkBlockItem extends BlockItem {
    ...
    public @NotNull ItemStack finishUsingItem(ItemStack itemStack, Level level, LivingEntity livingEntity) {
        ...
        if (livingEntity instanceof ServerPlayer serverPlayer) {
            int quality = itemStack.has(DataComponents.CUSTOM_DATA)
                    && itemStack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().contains("brewery.beer_quality")
                    ? itemStack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getIntOr("brewery.beer_quality", 1)
                    : 1;
    ...
    public void appendHoverText(@NotNull ItemStack stack, @NotNull Item.TooltipContext context, @NotNull TooltipDisplay display, @NotNull Consumer<Component> tooltip, @NotNull TooltipFlag flag) {
        ...
        if (stack.has(DataComponents.CUSTOM_DATA)) {
            ...
            if (customData.copyTag().contains("brewery.beer_quality")) {
                beerQuality = customData.copyTag().getIntOr("brewery.beer_quality", 1);
            }
        }
        ...
        if (this.effect != null) {
            ...
            tooltip.accept(line.withStyle(this.effect.getCategory().getTooltipFormatting()));
        } else {
            tooltip.accept(Component.translatable("effect.none").withStyle(ChatFormatting.GRAY));
        }

        if (beerQuality != 1) {
            tooltip.accept(Component.translatable("tooltip.brewery.beer_quality", beerQuality).withStyle(ChatFormatting.GOLD));
        }
    }
}
