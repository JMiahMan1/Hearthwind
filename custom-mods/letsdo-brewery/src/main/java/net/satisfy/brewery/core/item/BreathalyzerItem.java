...
public class BreathalyzerItem extends Item {
    ...
    public static void init() {
        ...
        // 26.2: code-registered item model predicates are gone (vanilla ItemProperties and
        // architectury ItemPropertiesRegistry were both removed). The "breathing" and
        // "drunkenness" predicates must become data-driven range_select/select item models
        // in resources (out of scope for this port). Kept as a no-op so client init callers
        // still compile.
    }
    ...
    public void appendHoverText(@NotNull ItemStack itemStack, @NotNull Item.TooltipContext tooltipContext, @NotNull net.minecraft.world.item.component.TooltipDisplay display, @NotNull java.util.function.Consumer<Component> tooltip, @NotNull TooltipFlag tooltipFlag) {
        if (itemStack.has(DataComponents.CUSTOM_DATA)) {
            String drunkennessValue = itemStack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getStringOr("brewery.drunkenness", "");
            Component line = switch (drunkennessValue) {
