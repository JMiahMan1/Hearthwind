...
public class ArmorRegistry {
    ...
    public static void appendToolTip(@NotNull List<Component> tooltip) {
        ...
        boolean hasFullSet = isHat(helmet) &&
                isChest(chestplate) &&
                isLegs(leggings) &&
                isBoots(boots);
    ...
    private static boolean isHat(ItemStack stack) {
        return stack.is(ObjectRegistry.BREWFEST_HAT.get()) || stack.is(ObjectRegistry.BREWFEST_HAT_RED.get());
    }

    private static boolean isChest(ItemStack stack) {
        return stack.is(ObjectRegistry.BREWFEST_REGALIA.get()) || stack.is(ObjectRegistry.BREWFEST_BLOUSE.get());
    }

    private static boolean isLegs(ItemStack stack) {
        return stack.is(ObjectRegistry.BREWFEST_TROUSERS.get()) || stack.is(ObjectRegistry.BREWFEST_DRESS.get());
    }

    private static boolean isBoots(ItemStack stack) {
        return stack.is(ObjectRegistry.BREWFEST_BOOTS.get()) || stack.is(ObjectRegistry.BREWFEST_SHOES.get());
    }
