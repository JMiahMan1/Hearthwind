...
public class BlackoutEffect extends MobEffect {
    ...
    public void onEffectAdded(LivingEntity livingEntity, int amplifier) {
        ...
        var sound = state.getSoundType().getFallSound();
    ...
    public boolean applyEffectTick(ServerLevel level, LivingEntity livingEntity, int amplifier) {
        ...
        if (self != null) {
            ...
            if (remaining == TELEPORT_AT_REMAINING) {
                ...
                while (gy < level.getMaxY() - 2 && (!level.getBlockState(pos).isAir() || !level.getBlockState(pos.above()).isAir())) {
                    gy++;
                    pos.setY(gy);
                }
            ...
            if (remaining == 1) {
                if (!livingEntity.level().isClientSide()) {
                    livingEntity.addEffect(new MobEffectInstance(MobEffects.NAUSEA, 600, 0));
                }
                livingEntity.setPose(Pose.STANDING);
                LOCK_YAW.remove(livingEntity.getUUID());
            }
