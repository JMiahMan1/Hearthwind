...
public class ChickenFeedItem extends Item {
    ...
    public @NotNull InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity entity, InteractionHand hand) {
        if (PlatformHelper.isChickenEffectsEnabled() && !player.level().isClientSide() && entity.getType() == EntityTypes.CHICKEN) {
            Chicken chicken = (Chicken) entity;
            chicken.addEffect(new MobEffectInstance(MobEffectRegistry.getHolder(MobEffectRegistry.CLUCK), 1200));
            player.level().playSound(null, chicken.getX(), chicken.getY(), chicken.getZ(), net.satisfy.farm_and_charm.core.util.PetSounds.chickenAmbient(chicken), SoundSource.NEUTRAL, 1.0F, 1.0F);
    ...
    public @NotNull ItemStack finishUsingItem(ItemStack stack, Level world, LivingEntity entityLiving) {
        if (PlatformHelper.isChickenEffectsEnabled() && entityLiving instanceof Player player && !world.isClientSide()) {
            player.addEffect(new MobEffectInstance(MobEffectRegistry.getHolder(MobEffectRegistry.CLUCK), 400));
            world.playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.PLAYER_BURP, SoundSource.PLAYERS, 0.5F, world.getRandom().nextFloat() * 0.1F + 0.9F);
            if (!player.getAbilities().instabuild) {
                stack.shrink(1);
            }
        }
    ...
    public void appendHoverText(@NotNull ItemStack itemStack, @NotNull Item.TooltipContext tooltipContext, @NotNull net.minecraft.world.item.component.TooltipDisplay display, @NotNull java.util.function.Consumer<Component> tooltip, @NotNull TooltipFlag tooltipFlag) {
        if (PlatformHelper.isChickenEffectsEnabled()) {
            tooltip.accept(Component.translatable("tooltip.farm_and_charm.animal_fed_to_chicken").withStyle(ChatFormatting.GRAY));
            tooltip.accept(Component.translatable("tooltip.farm_and_charm.cluck").withStyle(ChatFormatting.BLUE));
        }
    }
}