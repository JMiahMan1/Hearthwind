...
import net.satisfy.farm_and_charm.core.util.PetSounds;
...
public class WhineAtBowlGoal extends Goal {
    ...
    // 26.2: WOLF_WHINE/WOLF_PANT constants moved into the wolf's own sound
    // variant; playWhineSound keeps alternating between the same two sounds.
    private SoundEvent pickWhineSound() {
        return wolf.getRandom().nextBoolean() ? PetSounds.wolfWhine(wolf) : PetSounds.wolfPant(wolf);
    }
    ...
    public boolean canUse() {
        ...
        if (!isValidWhineTime(server.getGameTime())) return false;
    ...
    public void tick() {
        ...
        if (owner != null && wolf.distanceTo(owner) < 4.0 && server.getRandom().nextInt(100) < 10) {
            wolf.playSound(net.satisfy.farm_and_charm.core.util.PetSounds.wolfWhine(wolf), 0.6f, 0.9f + server.getRandom().nextFloat() * 0.2f);
        }
        ...
        if (++whineTicks >= MAX_WHINE_TICKS) {
            ...
            if (currentBowl instanceof PetBowlBlockEntity finalBowl && finalBowl.isEmpty()) {
                wolf.playSound(net.satisfy.farm_and_charm.core.util.PetSounds.wolfGrowl(wolf), BASE_VOLUME, BASE_PITCH);
                Vec3 pos = wolf.position().add(0, 0.5, 0);
    ...
    private void playWhineSound() {
        SoundEvent sound = pickWhineSound();
