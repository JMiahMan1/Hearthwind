...
public class MeowAtBowlGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        long timeOfDay = server.getGameTime() % 24000;
    ...
    public void tick() {
        ...
        if (meowTicks % MEOW_INTERVAL == 0) {
            cat.playSound(net.satisfy.farm_and_charm.core.util.PetSounds.catBeg(cat), 1.0f, 1.0f);
        }
        ...
        if (++meowTicks >= MAX_MEOW_TICKS) {
            cat.playSound(net.satisfy.farm_and_charm.core.util.PetSounds.catHiss(cat), 1.0f, 1.0f);
            Vec3 pos = cat.position().add(0, LOOK_OFFSET, 0);
