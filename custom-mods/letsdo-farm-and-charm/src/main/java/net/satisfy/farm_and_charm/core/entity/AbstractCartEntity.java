...
public abstract class AbstractCartEntity extends Entity {
    ...
    // 26.2: Entity.getBoundingBoxForCulling removed (culling is client-renderer side now).
    // Kept as a plain helper with identical bounds for the cart renderers.
    ...
    protected void readAdditionalSaveData(net.minecraft.world.level.storage.ValueInput input) {
        this.pullingUuid = input.read("PullingUUID", net.minecraft.core.UUIDUtil.CODEC).orElse(null);
        this.entityData.set(DATA_SPEED, input.getFloatOr("CartSpeed", 0.0F));
        this.entityData.set(DATA_DAMAGE, input.getFloatOr("CartDamage", 0.0F));
    }

    @Override
    protected void addAdditionalSaveData(net.minecraft.world.level.storage.ValueOutput output) {
        if (this.pulling != null && this.pullingUuid != null) {
            output.store("PullingUUID", net.minecraft.core.UUIDUtil.CODEC, this.pullingUuid);
        }
        output.putFloat("CartSpeed", this.entityData.get(DATA_SPEED));
        output.putFloat("CartDamage", this.entityData.get(DATA_DAMAGE));
    }
    ...
    @Override
    public @NotNull InteractionResult interact(Player player, InteractionHand hand, net.minecraft.world.phys.Vec3 hitPos) {
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide() && this.getY() < this.level().getMinY() - 64) {
            this.destroyCart(null);
        }
    }

    // 26.2: Entity.hurt is final; cart damage now flows through hurtServer (full original logic preserved).
    @Override
    ...
    protected void destroyCart(@Nullable DamageSource source) {
        ...
        if (!this.level().isClientSide()) {
            if (this.level() instanceof net.minecraft.server.level.ServerLevel _sl) this.spawnAtLocation(_sl, this.getCartItemStack().copy());
        }
    }
