...
public class TeaJugItem extends BlockItem {
    ...
    public void appendHoverText(@NotNull ItemStack itemStack, @NotNull Item.TooltipContext tooltipContext, @NotNull net.minecraft.world.item.component.TooltipDisplay display, @NotNull java.util.function.Consumer<Component> tooltip, @NotNull TooltipFlag tooltipFlag) {
        ...
        if (list2.isEmpty()) {
            ...
            for (var _aeLoop : list2) { for (net.minecraft.world.effect.MobEffectInstance statusEffectInstance : _aeLoop.effects()) {
                ...
                statusEffect.createModifiers(statusEffectInstance.getAmplifier(), (holderx, attributeModifierx) -> {
                    AttributeModifier entityAttributeModifier = new AttributeModifier(
                            attributeModifierx.id(),
                            attributeModifierx.amount() * (double) (statusEffectInstance.getAmplifier() + 1),
                ...
                if (statusEffectInstance.getDuration() > 20) {
                    mutableText = Component.translatable(
                            "potion.withDuration",
                            mutableText, MobEffectUtil.formatDuration(statusEffectInstance, _aeLoop.probability(), tooltipContext.tickRate()));
                }

                tooltip.accept(mutableText.withStyle(statusEffect.getCategory().getTooltipFormatting())); }
            }
        }
        ...
        if (!list3.isEmpty()) {
            ...
            for (Pair<Holder<Attribute>, AttributeModifier> pair : list3) {
                ...
                if (d > 0.0) {
                    tooltip.accept(Component.translatable("attribute.modifier.plus." + entityAttributeModifier3.operation().id(), ItemAttributeModifiers.ATTRIBUTE_MODIFIER_FORMAT.format(e), Component.translatable(pair.getFirst().value().getDescriptionId())).withStyle(ChatFormatting.BLUE));
                } else if (d < 0.0) {
                    e *= -1.0;
                    tooltip.accept(Component.translatable("attribute.modifier.take." + entityAttributeModifier3.operation().id(), ItemAttributeModifiers.ATTRIBUTE_MODIFIER_FORMAT.format(e), Component.translatable(pair.getFirst().value().getDescriptionId())).withStyle(ChatFormatting.RED));
                }
            }
        }
    }
    ...
    public @NotNull ItemStack finishUsingItem(ItemStack stack, Level level, LivingEntity consumer) {
        ...
        if ((stack.getItem().getCraftingRemainder().item().value() != net.minecraft.world.item.Items.AIR)) {
            assert stack.getItem().getCraftingRemainder().item().value() != null;
            containerStack = new ItemStack(stack.getItem().getCraftingRemainder().item().value());
        }
