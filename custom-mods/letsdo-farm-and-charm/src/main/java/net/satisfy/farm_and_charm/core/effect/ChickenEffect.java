    ...
    public boolean applyEffectTick(net.minecraft.server.level.ServerLevel level, LivingEntity entity, int amplifier) {
        if (!entity.level().isClientSide()) {
            ...
            if (entity.tickCount % tickInterval == 0) {
                if (entity.level().getRandom().nextFloat() < eggChance) {
                ...
                if (entity.level().getRandom().nextFloat() < featherChance) {
                ...
                if (entity instanceof Player && entity.level().getRandom().nextFloat() < 0.1) {
                    entity.level().playSound(null, entity.getX(), entity.getY(), entity.getZ(), SoundEvents.CHICKEN_EGG, SoundSource.PLAYERS, 1.0F, 1.0F);
                }
            }
        }
        return true;
