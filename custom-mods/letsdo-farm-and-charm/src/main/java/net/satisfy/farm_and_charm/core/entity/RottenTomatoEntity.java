...
public class RottenTomatoEntity extends ThrowableItemProjectile {
    ...
    public RottenTomatoEntity(Level world, LivingEntity owner) {
        super(EntityTypeRegistry.ROTTEN_TOMATO.get(), owner, world, new ItemStack(ObjectRegistry.ROTTEN_TOMATO.get()));
    }
    ...
    private ParticleOptions getParticleParameters() {
        return new ItemParticleOption(ParticleTypes.ITEM, ObjectRegistry.ROTTEN_TOMATO.get());
    }
    ...
    public void handleEntityEvent(byte id) {
        if (id == 3) {
            ...
            for (int i = 0; i < 30; ++i) {
                double offsetX = this.level().getRandom().nextGaussian() * 0.02D;
                double offsetY = this.level().getRandom().nextGaussian() * 0.02D;
                double offsetZ = this.level().getRandom().nextGaussian() * 0.02D;
    ...
    protected void onHit(HitResult hitResult) {
        ...
        if (!this.level().isClientSide()) {
            ...
            if (this.level().getRandom().nextFloat() < 0.15F && this.level() instanceof ServerLevel sl) {
                this.spawnAtLocation(sl, ObjectRegistry.TOMATO_SEEDS.get());
            }
            this.discard();
        }
    }
