...
public class FurHelmetRenderer implements ArmorRenderer {
    ...
    public void render(PoseStack matrices, SubmitNodeCollector collector, ItemStack stack, HumanoidRenderState renderState, EquipmentSlot slot, int light, HumanoidModel<HumanoidRenderState> contextModel) {
        ...
        FurHelmetModel model = ArmorRegistry.getHatModel(furHelmetItem, contextModel.head);

        Identifier baseTexture = furHelmetItem.getHatTexture();
        ...
        collector.submitModel(model, renderState, matrices, RenderTypes.entityCutout(texture), light, OverlayTexture.NO_OVERLAY, EntityRenderState.NO_OUTLINE, null);
        matrices.popPose();
    }
}
