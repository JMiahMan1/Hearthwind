...
public class WoodcutterGuiHandler extends AbstractContainerMenu {
    ...
    // Client mirror of the available recipe results (26.2 clients cannot query
    // the recipe manager); filled by WoodcutterRecipesPayload.
    private List<ItemStack> clientResults = Lists.newArrayList();
    ...
    public int getAvailableRecipeCount() {
        return this.world.isClientSide() ? this.clientResults.size() : this.availableRecipes.size();
    }

    public ItemStack getClientResult(int index) {
        if (index >= 0 && index < this.clientResults.size()) {
            return this.clientResults.get(index);
        }
        return ItemStack.EMPTY;
    }

    public void setClientResults(List<ItemStack> results) {
        this.clientResults = List.copyOf(results);
    }

    public boolean canCraft() {
        return this.inputSlot.hasItem()
                && (this.world.isClientSide() ? !this.clientResults.isEmpty() : !this.availableRecipes.isEmpty());
    }
    ...
    private void updateInput(Container input, ItemStack stack) {
        ...
        if (this.world instanceof net.minecraft.server.level.ServerLevel serverLevel && !stack.isEmpty()) {
            this.availableRecipes = serverLevel.recipeAccess().getRecipes().stream()
                    .filter(h -> h.value() instanceof WoodcuttingRecipe)
                    .map(h -> (RecipeHolder<WoodcuttingRecipe>) h)
                    .toList();
            if (this.player instanceof net.minecraft.server.level.ServerPlayer serverPlayer) {
                List<ItemStack> results = this.availableRecipes.stream()
                        .map(h -> h.value().getResultItem())
                        .toList();
                net.fabricmc.fabric.api.networking.v1.ServerPlayNetworking.send(
                        serverPlayer, new WoodcutterRecipesPayload(this.containerId, results));
            }
        }
    }
    ...
    void populateResult() {
        if (!this.availableRecipes.isEmpty() && this.isInBounds(this.selectedRecipe.get())) {
            ...
            ItemStack itemStack = woodcuttingRecipe.value().assemble(new SingleRecipeInput(input.getItem(0)));
    ...
    private boolean hasWoodcuttingRecipe(ItemStack stack) {
        if (this.world instanceof net.minecraft.server.level.ServerLevel serverLevel) {
            return serverLevel.recipeAccess()
                    .getRecipeFor(RecipeRegistry.WOODCUTTING.get(), new SingleRecipeInput(stack), serverLevel)
                    .isPresent();
        }
        // Client mirrors the server list; any visible recipe means the held
        // stack matched on the server.
        return this.inputSlot.hasItem() && !this.clientResults.isEmpty();
    }
        ...
        if (slot.hasItem()) {
            ...
            if (index == 1) {
                ...
                slot.onQuickCraft(itemStack2, itemStack);
                    ...
                    (hasWoodcuttingRecipe(itemStack2) ? !this.moveItemStackTo(itemStack2, 0, 1, false) :
