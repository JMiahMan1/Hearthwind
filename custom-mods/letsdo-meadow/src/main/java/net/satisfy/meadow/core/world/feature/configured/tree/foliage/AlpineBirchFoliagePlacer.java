...
public class AlpineBirchFoliagePlacer extends FoliagePlacer {
    public static final MapCodec<AlpineBirchFoliagePlacer> CODEC = RecordCodecBuilder.mapCodec(instance ->
            foliagePlacerParts(instance)
                    .and(IntProviders.codec(0, 24).fieldOf("trunk_height").forGetter(placer -> placer.trunkHeight))
                    .apply(instance, AlpineBirchFoliagePlacer::new)
    );

    private final IntProvider trunkHeight;

    public AlpineBirchFoliagePlacer(IntProvider radius, IntProvider offset, IntProvider height) {
        super(radius, offset);
    ...
    protected @NotNull FoliagePlacerType<?> type() {
        return PlacerTypeRegistry.ALPINE_BIRCH_FOLIAGE_PLACER.get();
    }
    ...
    private int calculateFoliageRadius(int iteration, WorldGenLevel level, FoliageSetter foliageSetter, RandomSource random, TreeConfiguration config, FoliageAttachment attachment, int offset) {
    ...
    private void placeCornShape(WorldGenLevel level, FoliageSetter foliageSetter, RandomSource random, TreeConfiguration config, FoliageAttachment attachment, int offset) {
