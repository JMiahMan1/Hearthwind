...
public final class BlankCakeInteractionRecipe implements Recipe<BlankCakeInteractionInput> {
    ...
    @Override
    public @NotNull ItemStack assemble(BlankCakeInteractionInput input) {
    ...
    @Override
    public @NotNull RecipeSerializer<BlankCakeInteractionRecipe> getSerializer() {
        return RecipeTypeRegistry.BLANK_CAKE_INTERACTION_SERIALIZER.get();
    }

    @Override
    public @NotNull RecipeType<BlankCakeInteractionRecipe> getType() {
    ...
    public static final class Serializer {
        ...
        public static final RecipeSerializer<BlankCakeInteractionRecipe> SERIALIZER = new RecipeSerializer<>(CODEC, STREAM_CODEC);
    }

    public record Result(Identifier setBlock, StatePatch setState, Identifier giveItem, Identifier sound, boolean consumeOne, boolean particles, int cooldownTicks) {
            public static final Codec<Result> CODEC = RecordCodecBuilder.create(instance -> instance.group(
                    Identifier.CODEC.optionalFieldOf("set_block").forGetter(r -> Optional.ofNullable(r.setBlock)),
                    StatePatch.CODEC.optionalFieldOf("set_state").forGetter(r -> Optional.ofNullable(r.setState)),
                    Identifier.CODEC.optionalFieldOf("give_item").forGetter(r -> Optional.ofNullable(r.giveItem)),
                    Identifier.CODEC.optionalFieldOf("sound").forGetter(r -> Optional.ofNullable(r.sound)),
    ...
    private static final class ResultStream {
        private static final StreamCodec<RegistryFriendlyByteBuf, Result> CODEC = new StreamCodec<>() {
            ...
            public @NotNull Result decode(RegistryFriendlyByteBuf buffer) {
                Identifier setBlock = buffer.readBoolean() ? Identifier.STREAM_CODEC.decode(buffer) : null;
                StatePatch setState = buffer.readBoolean() ? StatePatchStream.CODEC.decode(buffer) : null;
                Identifier giveItem = buffer.readBoolean() ? Identifier.STREAM_CODEC.decode(buffer) : null;
                Identifier sound = buffer.readBoolean() ? Identifier.STREAM_CODEC.decode(buffer) : null;
            ...
            public void encode(RegistryFriendlyByteBuf buffer, Result value) {
                ...
                if (value.setBlock() != null) {
                    Identifier.STREAM_CODEC.encode(buffer, value.setBlock());
                }
                ...
                if (value.giveItem() != null) {
                    Identifier.STREAM_CODEC.encode(buffer, value.giveItem());
                }

                buffer.writeBoolean(value.sound() != null);
                if (value.sound() != null) {
                    Identifier.STREAM_CODEC.encode(buffer, value.sound());
                }
