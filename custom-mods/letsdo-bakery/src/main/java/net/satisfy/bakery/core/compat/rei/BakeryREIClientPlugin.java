...
public class BakeryREIClientPlugin {
    ...
    public static void registerDisplays(DisplayRegistry registry) {
        registry.beginFiller(net.minecraft.world.item.crafting.RecipeHolder.class)
                .filter(holder -> holder.value() instanceof BakingStationRecipe)
                .fill(holder -> new BakerStationDisplay((BakingStationRecipe) holder.value()));
    }

    public static List<Ingredient> ingredients(Recipe<RecipeInput> recipe, ItemStack stack) {
        List<Ingredient> l = new ArrayList<>(recipe instanceof BakingStationRecipe r ? r.getIngredients() : List.of());
