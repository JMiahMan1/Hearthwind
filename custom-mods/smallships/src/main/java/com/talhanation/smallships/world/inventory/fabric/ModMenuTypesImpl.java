...
public class ModMenuTypesImpl {
    ...
    private static <T extends AbstractContainerMenu> MenuType<T> register(String id, MenuType<T> menuType) {
        return Registry.register(BuiltInRegistries.MENU, Identifier.fromNamespaceAndPath(SmallShipsMod.MOD_ID, id), menuType);
    }

    static {
        entries.put("ship_container", register("ship_container",
                new ExtendedMenuType<>((syncId, inventory, data) -> ModMenuTypes.extendedShipContainerMenuTypeSupplier(syncId, inventory, data.ship()), ContainerUtilityImpl.ContainerMenuData.PACKET_CODEC)));
    }
}
