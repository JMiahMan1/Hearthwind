...
public interface Cannonable extends Ability {
    ...
    default void readCannonShipSaveData(CompoundTag tag) {
        if (tag.contains("CannonCount")) {
            this.setCannonCount(tag.getByte("CannonCount").orElse((byte) 0));
    ...
    default Identifier getTextureLocation() {
    ...
    default void cannonShipDestroyed(ServerLevel level, Ship ship){
        for(int i = 0; i < getCannonCount(); i++){
            ship.spawnAtLocation(level, new ItemStack(ModItems.CANNON), 4f);
        }
    }
