...
public class CannonPoofParticles extends CompoundParticles {
    ...
    protected void addStaticMainPoofParticles(int amount) {
        for (int i = 0; i < amount; i++) {
            ...
            if (this.color == null) {
                ...
                this.level.addParticle(new CustomPoofParticleOptions(c.getAsVector3f()), pos.x, pos.y, pos.z, v.x, v.y, v.z);
            }
        }
    }
    ...
    protected void addPoofForwardParticles(int amount) {
        for (int i = 0; i < amount; i++) {
            ...
            Vector3d v = new Vector3d(rand).mul(0.03).add(this.getNormalizedDirection().mul(Math.abs(this.random.nextGaussian()) * 0.3F));
