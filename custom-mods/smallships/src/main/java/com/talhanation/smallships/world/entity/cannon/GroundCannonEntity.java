...
public class GroundCannonEntity extends Minecart implements ICannon {
    ...
    private static final EntityDataAccessor<String> BARREL_UUID = SynchedEntityData.defineId(GroundCannonEntity.class, EntityDataSerializers.STRING);
    ...
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(BARREL_UUID, "");
        builder.define(DYE, "");
    }

    public Optional<UUID> getEntityInBarrelUUID() {
        String s = this.entityData.get(BARREL_UUID);
        if (s == null || s.isEmpty()) return Optional.empty();
        try {
            return Optional.of(java.util.UUID.fromString(s));
        } catch (Exception e) {
    ...
    protected final void setEntityInBarrelUUID(UUID uuid) {
        this.entityData.set(BARREL_UUID, uuid == null ? "" : uuid.toString());
    }
    ...
    @Nullable
    private static DyeColor dyeColorOf(ItemStack stack) {
        String path = net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(stack.getItem()).getPath();
        if (path.endsWith("_dye")) path = path.substring(0, path.length() - 4);
    ...
    public void addAdditionalSaveData(ValueOutput output) {
        ...
        if (dye != null) output.putString("Dye", dye.getSerializedName());
    }

    @Override
    public void readAdditionalSaveData(ValueInput input) {
        super.readAdditionalSaveData(input);
        String dyeStr = input.getStringOr("Dye", "");
        if (!dyeStr.isEmpty()) {
            this.setDye(DyeColor.byName(dyeStr, null));
        }
    }
    ...
    protected boolean itemInteraction(Player player, InteractionHand interactionHand) {
        if (!this.level().isClientSide() && interactionHand == InteractionHand.MAIN_HAND) {
            ...
            if (item.getItem() instanceof DyeItem) {
                DyeColor itemColor = dyeColorOf(item);
                if (itemColor != null && !itemColor.equals(this.getDye())) {
                    item.shrink(1);
                    this.setDye(itemColor);
                }
                return true;
    ...
    public CannonBallItem getCannonBallToShoot() {
        ...
        if (this.getPassengerDriver() instanceof ICannonBallSource container) {
            ...
            for (int i = 0; i < player.getInventory().getContainerSize(); i++) {
                if (player.getInventory().getItem(i).is(ModItems.CANNON_BALL)) return ModItems.CANNON_BALL;
            }
            return null;
        } else {
            return null;
