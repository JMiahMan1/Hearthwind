...
public class CannonBallShootParticles extends CompoundParticles {
    ...
    protected void addFlamesForwardParticles(int amount) {
        for (int i = 0; i < amount; i++) {
            ...
            Vector3d v = new Vector3d(rand).mul(0.02).add(this.getNormalizedDirection().mul(Math.abs(this.random.nextGaussian()) * 0.2F));
    ...
    protected void addDarkSmokeParticles(int amount) {
        for (int i = 0; i < amount; i++) {
            Vector3d rand = VectorMath.getRandGaussian(this.random);
            Vector3d pos = new Vector3d(rand).mul(0.2).add(this.getPos());
            Vector3d v = new Vector3d(rand).mul(0.02).add(this.getNormalizedDirection().mul(Math.max(0, this.random.nextGaussian()) * 0.05F));
