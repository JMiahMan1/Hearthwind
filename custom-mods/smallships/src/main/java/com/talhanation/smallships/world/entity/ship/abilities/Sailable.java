...
public interface Sailable extends Ability {
    ...
    default void readSailShipSaveData(CompoundTag tag) {
        CompoundTag compoundTag = tag.getCompound("Sail").orElse(new CompoundTag());
        self().setData(SAIL_STATE, compoundTag.getByte("State").orElse((byte) 0));
        self().setData(Ship.SAIL_COLOR, compoundTag.getString("Color").orElse(""));
    ...
    default boolean interactSail(Player player, InteractionHand interactionHand) {
        ...
        for (DyeColor dc : DyeColor.values()) {
            if (item.getItem().toString().contains(dc.getName())) {
                String color = dc.getName();
