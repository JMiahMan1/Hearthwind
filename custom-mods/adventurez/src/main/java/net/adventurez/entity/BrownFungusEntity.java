...
public class BrownFungusEntity extends PathfinderMob implements NeutralMob {
    ...
    public BrownFungusEntity(EntityType<? extends PathfinderMob> entityType, Level level) {
    ...
    public static boolean canSpawn(EntityType<BrownFungusEntity> type, ServerLevelAccessor level, EntitySpawnReason spawnReason, BlockPos pos, RandomSource random) {
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide()) {
            ...
            int randomMushroom = this.level().getRandom().nextInt(20000);
            if (randomMushroom == 1997
                ...
                this.level().setBlock(this.blockPosition(), Blocks.BROWN_MUSHROOM.defaultBlockState(), 3);
            }
        }
    }
    ...
    public boolean doHurtTarget(ServerLevel level, Entity target) {
        if (!super.doHurtTarget((ServerLevel) this.level(), target)) {
            ...
            if (!this.level().isClientSide() && target instanceof LivingEntity && this.level().getRandom().nextInt(160) > 100) {
                ((LivingEntity) target).addEffect(new MobEffectInstance(MobEffects.POISON, 80 + this.level().getRandom().nextInt(100), 0, false, false), this);
            }
            return true;
        }
    }
