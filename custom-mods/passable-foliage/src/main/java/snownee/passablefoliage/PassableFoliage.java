...
/**
 * Fork-port of Snownee's Passable Foliage 26.1.1+fabric (26.1-fabric branch)
 * to 26.2, self-contained (upstream's Kiwi library has no 26.2 build, so the
 * Kiwi module/config/GO system is replaced with plain Fabric equivalents;
 * see {@link PassableFoliageCommonConfig}, {@link FoliageTags} and
 * {@link LeafWalker}). Collision behaviour is otherwise verbatim upstream.
 */
public final class PassableFoliage {
	...
	public static void onEntityCollidedWithLeaves(
		...
		if (!PassableFoliageCommonConfig.soundsPlayerOnly || entity instanceof Player) {
			if (blockState.is(BlockTags.LEAVES)) {
				...
				// 26.2: Level.getGameTime() moved to LevelData.
				else if (world.getLevelData().getGameTime() % 6 == 0) {
	...
	public static boolean hasLeafWalker(LivingEntity entity) {
		return PassableFoliageCommonConfig.alwaysLeafWalking || enchantmentEnabled && EnchantmentHelper.has(
				entity.getItemBySlot(
						EquipmentSlot.FEET), LeafWalker.TYPE);
	}
	...
	public static void tagsLoaded() {
		for (Block block : BuiltInRegistries.BLOCK) {
			try {
				((PassableFoliageBlock) block).pfoliage$setPassable(block.defaultBlockState().is(FoliageTags.PASSABLES));
			} catch (Throwable e) {
				if (!err) {
