...
public class BlockStateFeature implements MarkdownFeature {
    ...
    public void registerTokens(TokenRegistrar registrar) {
        registrar.registerToken((nibbler, tokens) -> {
            ...
            try {
                var parsed = BlockStateParser.parseForTesting(
                        (net.minecraft.core.HolderLookup<net.minecraft.world.level.block.Block>) (Object) BuiltInRegistries.BLOCK,
                        blockStateString, true
                );
                if (parsed.left().isEmpty()) return false;
                tokens.add(new BlockStateToken(blockStateString, parsed.left().get()));
                return true;
