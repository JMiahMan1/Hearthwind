...
public class ItemStackFeature implements MarkdownFeature {
    ...
    public ItemStackFeature() {}

    public ItemStackFeature(net.minecraft.core.HolderLookup.Provider ignored) {}
    ...
    public void registerTokens(TokenRegistrar registrar) {
        registrar.registerToken((nibbler, tokens) -> {
            ...
            try {
                ...
                var result = new ItemParser(level.registryAccess()).parse(new StringReader(itemStackString));
                var stack = new ItemStack(result.item(), 1);
                if (result.components() != null) stack.applyComponents(result.components());
