...
public class FlaskItem extends Item {
    ...
    private CustomModelData newRandomTexture(@Nullable Level level, ItemStack stack) {
        ...
        if (!stack.has(DataComponents.CUSTOM_MODEL_DATA)) {
            ...
            return new CustomModelData(List.of((float) randomTexture), List.of(), List.of(), List.of());
        } else {
    ...
    private void applyPotionEffects(ItemStack stack, Player player) {
        ...
        if (data.hasEffects()) {
            data.forEachEffect(player::addEffect, 1.0F);
        }
    }
    ...
    public void onCraftedBy(ItemStack stack, Player player) {
        ...
        stack.set(DataComponents.CUSTOM_MODEL_DATA, newRandomTexture(player.level(), stack));
    }

    @Override
    public void appendHoverText(@NotNull ItemStack stack, @NotNull Item.TooltipContext tooltipContext, @NotNull TooltipDisplay display, @NotNull Consumer<Component> tooltip, @NotNull TooltipFlag context) {
        if (stack.has(DataComponents.POTION_CONTENTS)) {
            ...
            potionContents.forEachEffect(mobEffectInstance -> {
                ...
                tooltip.accept(effectName.withStyle(mobEffectInstance.getEffect().value().getCategory().getTooltipFormatting()));
            }, 1.0F);
        } else {
        ...
        if (!list3.isEmpty()) {
            ...
            for (Pair<Attribute, AttributeModifier> pair : list3) {
                ...
                if (d > 0.0) {
                    tooltip.accept(Component.translatable("attribute.modifier.plus." + modifier.operation().id(), ItemAttributeModifiers.ATTRIBUTE_MODIFIER_FORMAT.format(e), Component.translatable(pair.getFirst().getDescriptionId())).withStyle(ChatFormatting.BLUE));
                } else if (d < 0.0) {
                    e *= -1.0;
                    tooltip.accept(Component.translatable("attribute.modifier.take." + modifier.operation().id(), ItemAttributeModifiers.ATTRIBUTE_MODIFIER_FORMAT.format(e), Component.translatable(pair.getFirst().getDescriptionId())).withStyle(ChatFormatting.RED));
                }
            }
        }
    }
