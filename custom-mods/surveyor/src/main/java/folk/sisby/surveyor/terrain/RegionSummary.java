...
public class RegionSummary {
	...
	public static RegionSummary fromEmpty(File folder, WorldSummary summary, RegionPos regionPos) {
		return new RegionSummary(summary, new File(folder, "c.%d.%d.dat".formatted(regionPos.x(), regionPos.z())), regionPos, new ChunkSummary[RegionPos.CHUNK_SIZE][RegionPos.CHUNK_SIZE], new BitSet(RegionPos.CHUNK_AREA), new RegistryPalette<>(summary.manager().lookupOrThrow(Registries.BIOME)), new RegistryPalette<>(summary.manager().lookupOrThrow(Registries.BLOCK)));
	}
	...
	protected void readNbt(RegionPos pos, boolean bitsOnly) {
		Registry<Biome> biomeRegistry = summary.manager().lookupOrThrow(Registries.BIOME);
		Registry<Block> blockRegistry = summary.manager().lookupOrThrow(Registries.BLOCK);
		...
		CompoundTag chunksCompound = nbt.getCompound(KEY_CHUNKS).orElse(new CompoundTag());
		...
		this.biomePalette = new RegistryPalette<>(summary.manager().lookupOrThrow(Registries.BIOME));
		this.blockPalette = new RegistryPalette<>(summary.manager().lookupOrThrow(Registries.BLOCK));
		this.chunks = new ChunkSummary[RegionPos.CHUNK_SIZE][RegionPos.CHUNK_SIZE];
		ListTag biomeList = nbt.getList(KEY_BIOMES).orElse(new ListTag());
		...
		ListTag blockList = nbt.getList(KEY_BLOCKS).orElse(new ListTag());
	...
	public void save(boolean unload) {
		...
		Registry<Biome> biomeRegistry = summary.manager().lookupOrThrow(Registries.BIOME);
		Registry<Block> blockRegistry = summary.manager().lookupOrThrow(Registries.BLOCK);
		nbt.put(KEY_BIOMES, new ListTag(mapIterable(biomePalette.view(), b -> StringTag.valueOf(biomeRegistry.getId(b).toString()))));
		nbt.put(KEY_BLOCKS, new ListTag(mapIterable(blockPalette.view(), b -> StringTag.valueOf(blockRegistry.getId(b).toString()))));
		...
		nbt.putIntArray(KEY_BIOME_GRASS, mapIterable(biomePalette.view(), b -> b.getGrassColor(0, 0)).stream().mapToInt(i -> i).toArray());
		nbt.putIntArray(KEY_BLOCK_COLORS, mapIterable(blockPalette.view(), b -> b.defaultMapColor().col).stream().mapToInt(i -> i).toArray());
