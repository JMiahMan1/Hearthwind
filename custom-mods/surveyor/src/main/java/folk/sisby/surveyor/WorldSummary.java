...
public class WorldSummary {
	...
	private final RegistryAccess manager;
	...
	public WorldSummary(@Nullable MinecraftServer server, ResourceKey<Level> dimension, RegistryAccess manager, File folder) {
		...
		Surveyor.LOGGER.info("[Surveyor] Loading data for {}", dimension.identifier());
		...
		Surveyor.LOGGER.info("[Surveyor] Finished loading data for {}", dimension.identifier());
	}

	public static WorldSummary of(Level world) {
		if (world.isClientSide()) return SurveyorClient.tryGetSummary(world.dimension());
		ServerSummary summary = ServerSummary.of(world.getServer());
		return summary == null ? null : summary.getWorld(world.dimension());
	}
	...
	public void save(@Nullable Level world, File folder, boolean suppressLogs) {
		if (!isDirty()) return;
		folder.mkdirs();
	...
	public @Nullable Level world() {
		return server != null ? server.getLevel(dimension) : SurveyorClient.getWorld(dimension);
	}
