...
public class WorldLandmarks {
	...
	public static WorldLandmarks load(WorldSummary summary, File folder) {
		...
		if (landmarksFile.exists()) {
			try {
				...
				Surveyor.LOGGER.error("[Surveyor] Error loading landmarks file for {}.", summary.dimension().identifier(), e);
			}
		}
	...
	public static WorldLandmarks fromNbt(WorldSummary summary, CompoundTag nbt, File landmarksFile) {
		CompoundTag landmarks = nbt.getCompound(KEY_LANDMARKS).orElse(new CompoundTag());
		boolean dirty = false;
		...
		if (landmarks.keySet().stream().anyMatch(k -> k.contains(":"))) { // 0.X
			...
			try {
				for (String key : landmarks.keySet()) {
					...
					for (String coords : type.keySet()) {
						...
						UUID owner = landmark.contains("owner") ? UUIDUtil.CODEC.decode(NbtOps.INSTANCE, landmark.get("owner")).resultOrPartial(Surveyor.LOGGER::error).orElseThrow().getFirst() : GLOBAL;
						...
						Identifier id = landmark.getString("texture").map(Identifier::tryParse).orElse(typeId).withSuffix((dye == null ? "" : "/" + dye.getId()) + "/" + pos.getX() + (pos.getY() == 0 ? "" : "/" + pos.getY()) + "/" + pos.getZ());
			...
			if (!summary.isClient()) {
				CompoundTag removed = nbt.getCompound(KEY_REMOVED).orElse(new CompoundTag());
		...
		if (isDirty()) {
			...
			Util.ioPool().execute(() -> {
				try {
					...
					Surveyor.LOGGER.error("[Surveyor] Error writing landmarks file for {}.", summary.dimension().identifier(), e);
				}
			});
			dirty = false;
