...
public class SurveyorConfig extends WrappedConfig {
	...
	public static final class Builtins implements Section {
		...
		public Set<Holder<MapDecorationType>> recordIcons() {
			List<Holder<MapDecorationType>> decorations = StreamSupport.stream(BuiltInRegistries.MAP_DECORATION_TYPE.entrySet().spliterator(), false).toList();
			recordIcons.keySet().removeIf(s -> decorations.stream().noneMatch(e -> e.getRegisteredName().equals(s)));
			for (Holder<MapDecorationType> value : decorations) recordIcons.putIfAbsent(value.getRegisteredName(), true);
			return decorations.stream().filter(t -> recordIcons.get(t.getRegisteredName())).collect(Collectors.toSet());
		}
