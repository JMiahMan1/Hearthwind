...
| earlystage | Primitive start | `data/earlystage` (sieve drops, flint/steel era recipes) | 🟡 partial | **Surface `rock` + `flint` blocks shipped** (4+2 size variants × facing, weighted worldgen per original, shovel cycles variant), flint tools + rock/flint BlockItems, ore-piece recipes, steel ingot/nugget/block + assets, stonecutting rocks_from_stone + crafting cobblestone_from_rock. Missing: knapping minigame UI, sieve block entity, beginner-death forgiveness (`beginnerDeathCount:3`), tier integration |
