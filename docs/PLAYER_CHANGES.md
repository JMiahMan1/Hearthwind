...
- **Underground Biomes & Structures (2026-09-22)**:
  - **Profundis**: 11 cave biomes (Frozen, Mushroom, Molten, Amethyst, Black, Arid, Floral Lush, White, Painted, Dirt, Sparse Lush) with custom features (amethyst veins, cave pillars, shelf fungi, icicles, painted caves).
  - **UndergroundWorlds**: overworld underground regions replacing spider-caves/underground-jungle.
  - **Dungeons+**: dungeon destinations; **Moogs Nether/End Structures** via StructureLib.
  - **Lukis Grand Capitals**: upgraded village capitals datapack (pack_format 107).
  - **Desert Dungeon** (DungeonZ addon): desert dungeon variant + loot.
...
- **26.2 dependency exceptions**: Lithostitched is not in the Aged 3.1.2 index, but the shipped Nature's Spirit `2.3.0+26.2` build hard-requires it at runtime. C2ME is also not an Aged mod and is retained because modern 26.2 server chunk generation requires the multithreaded worldgen implementation. Hearthwind-native modules are retained and may diverge where they replace Aged systems.
- `letsdo-*` family builds green in-tree (candlelight/meadow API errors resolved since the last audit).
  ...
  - Local Aged-dep ports (2026-09-22): lavender, logbegone, pockets, couplings, memoryleakfix, async-locator, passable-foliage — build green, container gametests **296/296**, vendored, deployed to all three Prism instances (`update_prism.sh` + `verify_prism`). Commit `ba4677096`. (entitycollisionfpsfix was dropped in the 0.1.4 audit: it is not in the Aged index and nothing depends on it.)
  - Structures wave (2026-09-22): **profundis** yarn→mojmap port boot-green (218 mods, Done); adopted **undergroundworlds / dungeons+ / MoogsNether+End+StructureLib / ForgeConfigAPIPort / desert-dungeon / unnamed-desert / betterendcities-vanilla**; **lukis-grand-capitals** datapack loaded; DNL **deferred** (177-class); mrpack overrides + Prism Full/Minimal/Dev-Client synced.
